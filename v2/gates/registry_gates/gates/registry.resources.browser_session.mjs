#!/usr/bin/env node
// Per-resource gate for resource_id=browser_session. CI invokes this via the
...
const r = getResource('browser_session');
const { ok, hits } = checkForbiddenGone('browser_session');

if (ok) {
  console.log(`[${PALETTE.pass}] registry.resources.browser_session: forbidden_paths have no v1 shadows`);
  console.log(`         status=${r.status || 'design'} policy=${r.policy_id}`);
  process.exit(0);
}

console.error(`[${PALETTE.fail}] registry.resources.browser_session: v1 leftovers for forbidden_paths:`);
