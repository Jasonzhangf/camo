#!/usr/bin/env node
// Per-resource gate for resource_id=subscription. CI invokes this via the
...
const r = getResource('subscription');
const { ok, hits } = checkForbiddenGone('subscription');

if (ok) {
  console.log(`[${PALETTE.pass}] registry.resources.subscription: forbidden_paths have no v1 shadows`);
  console.log(`         status=${r.status || 'design'} policy=${r.policy_id}`);
  process.exit(0);
}

console.error(`[${PALETTE.fail}] registry.resources.subscription: v1 leftovers for forbidden_paths:`);
