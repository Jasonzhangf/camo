#!/usr/bin/env node
// Per-resource gate for resource_id=error_envelope. CI invokes this via the
...
const r = getResource('error_envelope');
const { ok, hits } = checkForbiddenGone('error_envelope');

if (ok) {
  console.log(`[${PALETTE.pass}] registry.resources.error_envelope: forbidden_paths have no v1 shadows`);
  console.log(`         status=${r.status || 'design'} policy=${r.policy_id}`);
  process.exit(0);
}

console.error(`[${PALETTE.fail}] registry.resources.error_envelope: v1 leftovers for forbidden_paths:`);
