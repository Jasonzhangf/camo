...
3. **仓库边界（强制）**：
   ...
   - `legacy-app` 仓库路径：`~/Documents/github/legacy-app`。
   - `camo` 只承载通用能力（runtime/会话/路径解析/CLI 基础设施），不承载具体业务编排与平台策略（如 XHS 业务流程、点赞/评论业务规则）。
   - 具体业务逻辑必须落在 `legacy-app`，不要把 `legacy-app` 业务代码合并回 `camo`。
   - 跨仓库发布顺序：先发 `@web-auto/camo`，再发依赖该版本的 `@web-auto/legacy-app`。
4. **执行链路约束（强制）**：
   - `legacy-app` 的用户操作必须统一基于 `camo CLI / camo runtime`，禁止旁路实现。
   ...
   - 若能力缺失，先在 `camo` 补齐能力并发布，再由 `legacy-app` 接入；禁止在 `legacy-app` 临时回退实现。
5. **统一业务流程（强制）**：
