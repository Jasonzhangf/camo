...
describe('config utilities', () => {
  describe('module exports', () => {
    it('should export required functions', async () => {
      ...
      assert.strictEqual(typeof config.resolveLegacyRoot, 'function');
      assert.strictEqual(typeof config.resolveProfilesDir, 'function');
    });
  });

  describe('resolveLegacyRoot', () => {
    it('prefers explicit CAMO_DATA_ROOT without appending .camo', async () => {
      const { resolveLegacyRoot } = await import('../../../src/utils/config.mjs');
      const resolved = resolveLegacyRoot({
        platform: 'win32',
        hasDDrive: true,
        env: { CAMO_DATA_ROOT: 'D:\\camo' },
      });
      assert.equal(resolved, path.win32.join('D:', 'camo'));
    });

    it('prefers explicit CAMO_HOME without appending .camo', async () => {
      const { resolveLegacyRoot } = await import('../../../src/utils/config.mjs');
      const resolved = resolveLegacyRoot({
        platform: 'win32',
        hasDDrive: true,
        env: { CAMO_HOME: 'D:\\camo' },
      });
      assert.equal(resolved, path.win32.join('D:', 'camo'));
    });

    it('uses CAMO_HOME as direct root path', async () => {
      const { resolveLegacyRoot } = await import('../../../src/utils/config.mjs');
      const resolved = resolveLegacyRoot({
        platform: 'darwin',
        env: { CAMO_HOME: '/tmp/custom-camo-home' },
      });
      assert.equal(resolved, '/tmp/custom-camo-home');
    });
    ...
    it('keeps backward compatibility for parent root input', async () => {
      ...
      const resolved = resolveLegacyRoot({
        platform: 'darwin',
        env: { CAMO_ROOT: '/tmp/portable-root' },
      });
      assert.equal(resolved, path.join('/tmp/portable-root', '.camo'));
    });

    it('defaults to D:\\camo on windows when D drive exists', async () => {
      const { resolveLegacyRoot } = await import('../../../src/utils/config.mjs');
      const resolved = resolveLegacyRoot({
        platform: 'win32',
        hasDDrive: true,
    ...
    it('falls back to ~/.camo on windows when D drive is missing', async () => {
      const { resolveLegacyRoot } = await import('../../../src/utils/config.mjs');
      ...
      assert.equal(resolved, path.win32.join(home, '.camo'));
    });
  });

  describe('resolveProfilesDir', () => {
    it('uses CAMO_PROFILE_ROOT when provided', async () => {
      const { resolveProfilesDir } = await import('../../../src/utils/config.mjs');
      const resolved = resolveProfilesDir({
        platform: 'win32',
        env: { CAMO_PROFILE_ROOT: 'E:\\profiles' },
      });
    ...
    it('defaults to <root>/profiles', async () => {
      ...
      const resolved = resolveProfilesDir({
        platform: 'darwin',
        env: { CAMO_DATA_ROOT: '/tmp/camo-data' },
      });
      assert.equal(resolved, '/tmp/camo-data/profiles');
    });
  });
