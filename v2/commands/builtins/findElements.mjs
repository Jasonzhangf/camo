// camo v2 builtin: `camo find-elements --selector <css>|--text <text> [--target <t_id>] [--profile <id>]`
...
export async function run(transport, parsed = {}, ctx = {}) {
  ...
  const reply = await sendCommand(transport, { cmd: 'find-elements', args: { profile, target, selector, text } });
  return { cmd: 'find-elements', profile, target: reply.payload?.target || target, count: reply.payload?.count ?? 0, elements: reply.payload?.elements ?? [], issuedAt: new Date().toISOString(), traceId: ctx.traceId || null };
}
