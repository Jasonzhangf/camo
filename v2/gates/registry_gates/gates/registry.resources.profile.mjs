#!/usr/bin/env node
// Per-resource gate for resource_id=profile. CI invokes this via the
...
const r = getResource('profile');
const { ok, hits } = checkForbiddenGone('profile');

if (ok) {
  console.log(`[${PALETTE.pass}] registry.resources.profile: forbidden_paths have no v1 shadows`);
  console.log(`         status=${r.status || 'design'} policy=${r.policy_id}`);
  process.exit(0);
}

console.error(`[${PALETTE.fail}] registry.resources.profile: v1 leftovers for forbidden_paths:`);
