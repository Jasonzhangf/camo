...
import { SearchPlatform, SearchResponse, SearchResult } from '../SearchEngine.js';

export class XHSSearch extends SearchPlatform {
  readonly name = 'xhs';
  readonly searchURL = 'https://edith.xiaohongshu.com/search_result';
  
  async search(query: string, options?: { maxResults?: number; timeout?: number }): Promise<SearchResponse> {
  ...
  async parseResultsFromPage(maxResults: number): Promise<SearchResult[]> {
    const script = `
      () => {
        const notes = [];
        const selectors = [
          '.search-result-container .note-item',
          '[class*="search"] [class*="note"]',
        ...
        elements.slice(0, ${maxResults}).forEach(el => {
          ...
          if (title || link) {
            notes.push({ title, url: link, author, likes });
          }
        });
    ...
    try {
      const rawResults = await this.browser!.executeJS(script) as any[];
