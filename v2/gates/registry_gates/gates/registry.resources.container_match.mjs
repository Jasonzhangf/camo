#!/usr/bin/env node
// Per-resource gate for resource_id=container_match. CI invokes this via the
...
const r = getResource('container_match');
const { ok, hits } = checkForbiddenGone('container_match');

if (ok) {
  console.log(`[${PALETTE.pass}] registry.resources.container_match: forbidden_paths have no v1 shadows`);
  console.log(`         status=${r.status || 'design'} policy=${r.policy_id}`);
  process.exit(0);
}

console.error(`[${PALETTE.fail}] registry.resources.container_match: v1 leftovers for forbidden_paths:`);
