// camo v2 builtin: `camo hover --selector <css>|--text <text> [--target <t_id>] [--profile <id>]`
...
export async function run(transport, parsed = {}, ctx = {}) {
  ...
  const reply = await sendCommand(transport, { cmd: 'hover', args: { profile, target, selector, text } });
  return { cmd: 'hover', profile, target: reply.payload?.target || target, selector, text, hovered: reply.payload?.hovered === true, issuedAt: new Date().toISOString(), traceId: ctx.traceId || null };
}
