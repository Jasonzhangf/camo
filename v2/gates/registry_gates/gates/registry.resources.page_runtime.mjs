#!/usr/bin/env node
// Per-resource gate for resource_id=page_runtime. CI invokes this via the
...
const r = getResource('page_runtime');
const { ok, hits } = checkForbiddenGone('page_runtime');

if (ok) {
  console.log(`[${PALETTE.pass}] registry.resources.page_runtime: forbidden_paths have no v1 shadows`);
  console.log(`         status=${r.status || 'design'} policy=${r.policy_id}`);
  process.exit(0);
}

console.error(`[${PALETTE.fail}] registry.resources.page_runtime: v1 leftovers for forbidden_paths:`);
