#!/usr/bin/env node
// Per-resource gate for resource_id=autoscript_action. CI invokes this via the
...
const r = getResource('autoscript_action');
const { ok, hits } = checkForbiddenGone('autoscript_action');

if (ok) {
  console.log(`[${PALETTE.pass}] registry.resources.autoscript_action: forbidden_paths have no v1 shadows`);
  console.log(`         status=${r.status || 'design'} policy=${r.policy_id}`);
  process.exit(0);
}

console.error(`[${PALETTE.fail}] registry.resources.autoscript_action: v1 leftovers for forbidden_paths:`);
