...
const ENDPOINT_KEYS = ['wsUrl', 'httpUrl'];

function rejectEndpointOverrides(value, source) {
  for (const key of ENDPOINT_KEYS) {
    if (Object.hasOwn(value, key)) {
      throw new CamoError({
        code: 'E_CONFIG_INVALID',
        details: { source, field: key, reason: 'daemon endpoint comes from active registration' },
      });
    }
  }
}
...
export function loadConfig(overrides = {}) {
  ...
  if (fs.existsSync(CONFIG_FILE)) {
    try {
      ...
      rejectEndpointOverrides(fileConfig, CONFIG_FILE);
  ...
  if (process.env.CAMO_WS_URL || process.env.CAMO_HTTP_URL) {
    throw new CamoError({
      code: 'E_CONFIG_INVALID',
      details: { source: 'environment', reason: 'daemon endpoint comes from active registration' },
    });
  }
  ...
  rejectEndpointOverrides(overrides, 'overrides');
