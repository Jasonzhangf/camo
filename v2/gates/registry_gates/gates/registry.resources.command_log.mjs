#!/usr/bin/env node
// Per-resource gate for resource_id=command_log. CI invokes this via the
...
const r = getResource('command_log');
const { ok, hits } = checkForbiddenGone('command_log');

if (ok) {
  console.log(`[${PALETTE.pass}] registry.resources.command_log: forbidden_paths have no v1 shadows`);
  console.log(`         status=${r.status || 'design'} policy=${r.policy_id}`);
  process.exit(0);
}

console.error(`[${PALETTE.fail}] registry.resources.command_log: v1 leftovers for forbidden_paths:`);
