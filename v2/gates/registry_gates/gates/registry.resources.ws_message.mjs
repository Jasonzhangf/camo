#!/usr/bin/env node
// Per-resource gate for resource_id=ws_message. CI invokes this via the
...
const r = getResource('ws_message');
const { ok, hits } = checkForbiddenGone('ws_message');

if (ok) {
  console.log(`[${PALETTE.pass}] registry.resources.ws_message: forbidden_paths have no v1 shadows`);
  console.log(`         status=${r.status || 'design'} policy=${r.policy_id}`);
  process.exit(0);
}

console.error(`[${PALETTE.fail}] registry.resources.ws_message: v1 leftovers for forbidden_paths:`);
