...
- Release state: implementation is on main as `14ca512` and `00b71ca`; the worktree branch `codex/camo-multi-profile-idle-20260820` commit `c35b851` is a redundant parallel snapshot and will be cleaned up after claim release. Skill documentation is committed on main; global install is updated; DSH review is complete with one P1 finding that was fixed in the follow-up commit.
...
- Fix: daemon now checks/starts only the target profile, releases only the target profile, and projects `/health` and `/status` from all session details. Removed dead browser-state plumbing from command handlers and isolated progress-event roots in lifecycle tests.
- Verification: `npm run test:all` 377 unit, 10 smoke, 40 integration, 4 e2e pass on this Mac with isolated test roots; strict registry 19/19; build, file-size, diff checks pass; real CLI same-daemon A+B smoke returned independent Example Domain page info for both profiles.
- DSH review: `camo-multiprofile-engine-20260820-r4` correctly failed on stale mainline symbol and a non-hermetic pre-existing bootstrap test; fixed symbol to `releaseProfileBrowser`, regenerated docs, and confirmed the bootstrap test's lock isolation. Re-review required before commit.
